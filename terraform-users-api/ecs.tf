resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/${var.app_name}-users-api"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "${var.app_name}-users-api"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/${var.app_name}-users-api"
...
resource "aws_ecs_service" "main" {
  name            = "${var.app_name}-users-api-service"
  ...
  load_balancer {
    ...
    container_name   = "${var.app_name}-users-api"
...
resource "aws_appautoscaling_policy" "main" {
  name               = "${var.app_name}-users-api-cpu-scaling"
