...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "USERS_API_URL",     value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["users"]}" },
      { name = "PATIENT_API_URL",   value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["patient"]}" },
      { name = "FACILITY_API_URL",  value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["facility"]}" },
      { name = "GUARANTOR_API_URL", value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["guarantor"]}" },
      { name = "INVENTORY_API_URL", value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["inventory"]}" },
      { name = "REPORTS_API_URL",   value = "http://${data.terraform_remote_state.infra.outputs.api_alb_dns["reports"]}" },
    ]
    secrets = [
...
resource "aws_appautoscaling_policy" "main" {
  name               = "${var.app_name}-webapp-cpu-scaling"
