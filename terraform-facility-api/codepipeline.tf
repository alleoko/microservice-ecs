resource "aws_codestarconnections_connection" "main" {
  name          = "${var.app_name}-facility-api-github"
  provider_type = "GitHub"
  tags          = local.common_tags
}

resource "aws_codebuild_project" "main" {
  name          = "${var.app_name}-facility-api-build"
  build_timeout = 20
  ...
  environment {
    ...
    environment_variable { name = "CONTAINER_NAME",     value = "${var.app_name}-facility-api" }
  }

  source { type = "CODEPIPELINE", buildspec = "buildspec.yml" }
  logs_config { cloudwatch_logs { group_name = "/codebuild/${var.app_name}-facility-api", stream_name = "build" } }
  tags = local.common_tags
}

resource "aws_codepipeline" "main" {
  name     = "${var.app_name}-facility-api-pipeline"
