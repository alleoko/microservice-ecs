resource "aws_cloudwatch_log_group" "main" {
  name              = "/ecs/${var.app_name}-guarantor-api"
  retention_in_days = 14
  tags              = local.common_tags
}

resource "aws_ecs_task_definition" "main" {
  family                   = "${var.app_name}-guarantor-api"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([{
    name      = "${var.app_name}-guarantor-api"
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/${var.app_name}-guarantor-api"
...
resource "aws_ecs_service" "main" {
  name            = "${var.app_name}-guarantor-api-service"
  ...
  load_balancer {
    target_group_arn = "data.terraform_remote_state.infra.outputs.api_target_group_arns["guarantor"]"
    container_name   = "${var.app_name}-guarantor-api"
...
resource "aws_appautoscaling_policy" "main" {
  name               = "${var.app_name}-guarantor-api-cpu-scaling"
