...
output "webapp_alb_arn"       { value = aws_lb.webapp.arn }
...
output "api_alb_dns"          { value = { for k, v in aws_lb.api : k => v.dns_name } }
output "api_alb_arns"         { value = { for k, v in aws_lb.api : k => v.arn } }
